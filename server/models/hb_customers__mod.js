/*-----------------------------------------------------------------------------------------------*/
/*     модель данных для справочника "Заказчики"                                                 */
/*-----------------------------------------------------------------------------------------------*/
    ...
    try{
        ...
        switch (request_body.action) {
            case 'read':
                ...
                switch (target) {
                    case 'all':
                        return `SELECT id, customer, customer_alias, customer_fullname, customer_coef, 
                                        replace(replace(delete_mark::text, 'false', '-- --'), 'true', 'del') as delete_mark                                
                                FROM views.hb_customers__central_table where delete_mark = false`;
                    case 'cent':
                        return `SELECT id, customer, customer_alias, customer_fullname, customer_coef, 
                                        replace(replace(delete_mark::text, 'false', '-- --'), 'true', 'del') as delete_mark                                
            ...
            case 'insert':
                switch (request_body.target) {
                    case 'block_one_attachment':
                        request_body.data["parent_record"] = request_body.data.customer;
                        delete request_body.data["customer"];
                        request_body.data["table_id"] = 11111;
                    ...
                    case 'block_one_contact':
                        return `INSERT INTO data.customer_contacts ${data.headers} VALUES ${data.values} RETURNING *`;
                    case 'block_one_contact_person':
                        return `INSERT INTO data.customer_contact_persons ${data.headers} VALUES ${data.values} RETURNING *`;
                    case 'block_one_contact_person_contact':
            ...
            case 'update':
                switch (request_body.target) {
                    case 'block_customer':
                        return `UPDATE views.hb_customers__tab_main SET ${data} ${conditions} RETURNING *`;
                    case 'block_one_attachment':
                        return `UPDATE data.attachments SET ${data} ${conditions} RETURNING *`;
                    case 'block_one_contact':
                        return `UPDATE data.customer_contacts SET ${data} ${conditions} RETURNING *`;
                    case 'block_one_contact_person':
                        return `UPDATE data.customer_contact_persons SET ${data} ${conditions} RETURNING *`;
                    case 'block_one_contact_person_contact':
...
exports.get_helpers = async function (request_body, rec){
    ...
    try{
        ...
        let identity_docs = identity_docs_func.get_html(rec.docs,"Добавить документ", "customer", identity_block_attributes, request_body)
        let accounts = accounts_func.get_html(rec.accounts,"Добавить банк", "customer", identity_block_attributes) // ++KHC 19052022 Добавляем блок "Реквизиты"
